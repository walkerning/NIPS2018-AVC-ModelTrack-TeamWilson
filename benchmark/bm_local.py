...
def main(reader, types, save, forward_cfg, backward_cfg, verbose=False, addi_name=None):
    forward_model = create_fmodel_(forward_cfg)

    if any([n.endswith("transfer") for n in types]):
        backward_model = create_fmodel_(backward_cfg)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--backward-model", required=True)
    ...
    main(reader, args.type, args.save is not None, args.forward_model, args.backward_model, verbose=args.verbose)

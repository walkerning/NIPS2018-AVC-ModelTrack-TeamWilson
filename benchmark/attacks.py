import numpy as np
import foolbox as _foolbox

def pgd_03_001_40_re_transfer_attack(model, image, label, verbose=False):
    criterion = _foolbox.criteria.Misclassification()
    attack = _foolbox.attacks.PGD(model, criterion)
...
def iterative_transfer_attack(model, image, label, verbose=False):
    criterion = _foolbox.criteria.Misclassification()
    attack = _foolbox.attacks.L2BasicIterativeAttack(model, criterion)
...
def transfer_attack(model, image, label, verbose=False):
    criterion = _foolbox.criteria.Misclassification()
    attack = _foolbox.attacks.GradientAttack(model, criterion)
...
def saltnpepper_attack(model, image, label, verbose=False):
    criterion = _foolbox.criteria.Misclassification()
    attack = _foolbox.attacks.SaltAndPepperNoiseAttack(model, criterion)
    return attack(image, label, epsilons=50, repetitions=10)

def boundary_attack(model, image, label, verbose=False):
    criterion = _foolbox.criteria.Misclassification()
    init_attack = _foolbox.attacks.BlendedUniformNoiseAttack(model, criterion)
    ...
    if init_adversarial is None:
        ...
        attack = _foolbox.attacks.BoundaryAttack(model, criterion)
